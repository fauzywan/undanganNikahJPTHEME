...
// Komponen Loading sementara untuk Suspense per-section
const SectionLoader = () => (
  <div className="h-32 flex items-center justify-center text-[var(--gold)] opacity-50">
    Memuat bagian...
  </div>
);
...
const Undangan = () => {
  ...
  // ==========================================
  // 1. INITIAL FETCH DENGAN OPTIMASI (Conditional Fetching)
  // ==========================================
  useEffect(() => {
    // Hanya fetch data cerita jika belum ada di Redux
  ...
  useEffect(() => { 
    // Hanya fetch config jika belum ada di Redux
    if (!data || !data.config) {
      dispatch(fetchConfig()); 
    }

    // Logika RSVP tetap berjalan untuk mengecek tamu spesifik
    if (guestSlug) {
      dispatch(fetchGuestBySlug(guestSlug)).unwrap()
        .then((guest) => {
          if ((guest.isAttending !== undefined && guest.headcount > 0) || guest.message) {
            ...
            setIsRegistered(guestSlug !== '');
          }
        })
        .catch((err) => console.log("Info Tamu:", err.message));
    }
  }, [dispatch, data, guestSlug]);
  ...
  const getBgmUrl = () => {
    ...
    if (dbUrl && dbUrl.startsWith('/uploads/')) return `https://undangannikaharirini.onrender.com${dbUrl}`;
    return '/music/bgm.mp3'; 
  };
  ...
  const submitRSVP = async (e) => {
    ...
    try {
      if (!targetSlug) {
        const createRes = await axios.post(`${API_URL}/admin/guests`, { guestName: rsvpData.name });
  ...
  // Pastikan render loading ini hanya muncul kalau data benar-benar kosong di first load
  if (isLoading && (!data || !data.config)) {
    return (
  ...
  const year = eventDateObj != null ? eventDateObj.getFullYear() : new Date().getFullYear();
  const formattedDate = eventDateObj ? eventDateObj.toLocaleDateString('id-ID', { day: 'numeric', month: 'long' }) : 'Tanggal Belum Ditentukan';

  return (
    <>
      <style>{`
        /* Note: Pindahkan @import url(...) Google Fonts ke public/index.html agar tidak memblokir render */
        :root {
      ...
      {/* OPTIMASI: preload='metadata' sangat menghemat data awal */}
      <audio 
        ref={audioRef} 
        src={getBgmUrl()} 
        preload="metadata"
      ...
      {isOpened && (
        <>
          ...
          <div className="fixed right-4 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-50">
            <button onClick={scrollToPrevSection} className="w-10 h-10 flex items-center justify-center bg-white/70 backdrop-blur-sm border border-[var(--gold)] text-[var(--deep-red)] rounded-full shadow-lg transition-transform hover:scale-110 hover:bg-white" aria-label="Scroll ke atas">
            ...
            <button onClick={scrollToNextSection} className="w-10 h-10 flex items-center justify-center bg-white/70 backdrop-blur-sm border border-[var(--gold)] text-[var(--deep-red)] rounded-full shadow-lg transition-transform hover:scale-110 hover:bg-white" aria-label="Scroll ke bawah">
      ...
      <div id="main-content" style={{ opacity: fusumaProgress > 0.8 ? (fusumaProgress - 0.8) / 0.2 : 0, display: isOpened ? 'block' : 'none' }}>
        ...
        {/* OPTIMASI: Pecah Suspense menjadi beberapa blok */}
        ...
        <Suspense fallback={<SectionLoader />}>
          <div ref={el => sectionRefs.current[1] = el}>
            ...
            <CountdownSection eventDate={config?.events?.find(item => item.name.toLowerCase().includes("akad") || item.name.toLowerCase().includes("resepsi"))?.date || '2026-06-01'}  bride={config?.bride?.name} groom={config?.groom?.name}/>
          </div>
        ...
        <Suspense fallback={<SectionLoader />}>
          {photos && photos.filter(item => item.role === 'gallery').length > 0 && (
